    ...
    <div className='space-y-4 mt-2 rounded-md'>
      <div className='bg-gray-100 flex gap-7 items-center p-2 rounded-md'>
        ...
        <div className='space-y-2'>
          <h1 className='font-bold md:text-xl text-lg line-clamp-3 md:w-full w-[220px]'>{product.title}</h1>
          <p className='font-semibold flex items-center md:text-lg text-sm'>$<span className='md:text-4xl text-3xl'>{Math.floor(product.price)}</span> <span>({Math.floor(Math.random() * (30 - 2 + 1))}</span> % off)</p>
