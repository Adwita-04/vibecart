  ...
  return (
    <div className="bg-white py-3 shadow-2xl px-4 md:px-0">
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        {/* logo section */}
        <div className="flex gap-7 items-center flex-shrink-0">
          <Link to={"/"}>
            <h1 className="font-bold text-3xl">
              <span className="text-fuchsia-600 font-serif text-5xl">V</span>
              ibeCart
            </h1>
          </Link>
          ...
          {openDropdown ? (
            <div className="w-[250px] h-max z-50 shadow-2xl bg-white fixed top-16 left-60 border-2 p-5 border-gray-100 rounded-md">
              ...
              >
                {" "}
                <span onClick={toggleDropdown}>Detect my location</span>{" "}
              </button>
            </div>
          ) : null}
        </div>
        {/* menu section */}
        <nav className="flex items-center gap-7 flex-1 justify-end">
          <ul className="desktop-nav gap-7 items-center text-xl font-semibold">
            <li>
            ...
            <li>
              <NavLink to="/products" className={({ isActive }) => `${isActive ? "border-b-3 transition-all border-red-500" : "text-black"} cursor-pointer`}>
                Products
              </NavLink>
            </li>
            <li>
